...
impl TableStore {
    ...
    pub(crate) async fn open_sst(&self, id: &SsTableId) -> Result<SsTableHandle, SlateDBError> {
        ...
        // Read the block-format version from the self-describing footer so the
        // resulting handle carries the correct version (backport of upstream
        // #1341). Used when the manifest entry has no format_version.
        let format_version = self.sst_format.read_version(&obj).await?;
        Ok(SsTableHandle::new(*id, format_version, info))
    }
